  ...
  Spec generate() {
    ...
    return Class(
      (b) => b
        ..name = 'RouteNames'
        ..fields.addAll(
          pageRoutes.where((pageRoute) => !pageRoute.routeNameContainsParameters).map(
            (pageRoute) {
              final url =
              ...
              return Field(
                (b) => b
                  ..name = CaseUtil(pageRoute.routeName).camelCase
                  ..static = true
                  ..docs.add('/// $url')
        ...
        ..methods.addAll(
          pageRoutes.where((pageRoute) => pageRoute.routeNameContainsParameters).map(
            (pageRoute) {
              final parameters = pageRoute.routeName.parametersFromRouteName.map(
                (parameter) {
                  final argument = pageRoute.parameters.firstWhereOrNull((element) => element.name == parameter);
                  if (argument == null) {
                  ...
                  return Parameter(
                    (b) => b
                      ...
                      ..named = true
                      ..type = Reference('String${argument?.isRequired != true ? '?' : ''}')
