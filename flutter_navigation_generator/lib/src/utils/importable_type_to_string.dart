...
class ImportableTypeToStringUtils {
  static String convertFromString(ImportableType p, String s) {
    return switch (p.className) {
      ...
      'num' => 'num.parse($s)',
      'String' || 'dynamic' => s,
      'List' => '(jsonDecode(utf8.decode(base64Decode($s))) as List<dynamic>).map((e) => ${convertTypeArguments(p.typeArguments.first)}).toList()',
      'Map' => convertMap(p, s),
  ...
  static String convertTypeArguments(ImportableType p) {
    ...
    return switch (p.className) {
      ...
      'dynamic' => 'e',
      'Map' => convertMap(p, ('e')),
      'List' => '(e as List<dynamic>).map((e) => ${convertTypeArguments(p.typeArguments.first)}).toList()',
      _ => '${p.isNullable ? 'e == null ? null : ' : ''}${p.className}.fromJson(e as Map<String, dynamic>)',
    };
  }
  ...
  static String convertMap(ImportableType p, String s) {
    ...
    final mapFrom = 'Map<${typeKey.className}${typeKey.isNullable ? '?' : ''}, ${typeValue.className}${typeValue.isNullable ? '?' : ''}>.from';
    ...
    if (keyTransformer.length == 1 && valueTransformer.length == 1) return '$mapFrom($decode)';
  ...
  static String convertMapTransformer(ImportableType p, String s) {
    return switch (p.className) {
      ...
      'Map' => '${p.isNullable ? '$s == null ? null : ' : ''}${convertMap(p, s)}',
      'List' => '${p.isNullable ? '$s == null ? null : ' : ''}($s as List<dynamic>).map((e) => ${convertTypeArguments(p.typeArguments.first)}).toList()',
      _ => '${p.isNullable ? '$s == null ? null : ' : ''}${p.className}.fromJson($s as Map<String, dynamic>)',
    };
  }
}
