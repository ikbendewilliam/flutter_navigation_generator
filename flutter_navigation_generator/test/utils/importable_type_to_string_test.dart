...
void main() {
  String convertType(String className) => ImportableTypeStringConverter.convertFromString(ImportableType(className: className), 's');
  ...
  ImportableType? typeFromString(String string) {
    ...
    if (match == null) {
      return ImportableType(className: string.replaceAll('?', ''), isNullable: string.endsWith('?'));
    }
    ...
    if (typeValue == null) {
      return ImportableType(className: 'List', typeArguments: [typeFromString(typeKey)].nonNulls.toList());
    }
    return ImportableType(className: 'Map', typeArguments: [typeFromString(typeKey), typeFromString(typeValue)].nonNulls.toList());
  }

  void checkMap(String type, String expected) {
    final importableType = typeFromString(type)!;
    final result = ImportableTypeStringConverter.convertFromString(importableType, 's');
    expect(result, expected);
  }
  ...
  group('Lists', () {
    ...
    test('List with custom type that might be nullable', () {
      ...
      const expectNullable = '(jsonDecode(utf8.decode(base64Decode(s))) as List<dynamic>).map((e) => e == null ? null : CustomObject.fromJson(e as Map<String, dynamic>)).toList()';
