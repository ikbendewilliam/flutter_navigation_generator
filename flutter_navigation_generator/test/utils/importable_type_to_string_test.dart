...
void main() {
  ...
  ImportableType? typeFromString(String string) {
    ...
    if (typeValue == null) {
      return ImportableType(
        className: 'List',
        typeArguments: [typeFromString(typeKey)].nonNulls.toList(),
      );
    }
    return ImportableType(
      className: 'Map',
      typeArguments:
          [
            typeFromString(typeKey),
            typeFromString(typeValue),
          ].nonNulls.toList(),
    );
  }
  ...
  void checkMap(String type, String expected) {
    ...
    final result = ImportableTypeStringConverter.convertFromString(
      importableType,
      's',
    );
    expect(result, expected);
  ...
  group('Maps & Lists go deeper', () {
    ...
    test('Map with map with map', () {
      ...
      checkMap(
        'Map<Map<Map<String, dynamic>, dynamic>, String>',
    ...
    test('Map with List', () {
      ...
      checkMap(
        'Map<List<CustomObject>, CustomObject>',
    ...
    test('List with Map', () {
      ...
      checkMap(
        'List<Map<CustomObject, CustomObject>>',
        'Map<CustomObject, CustomObject>.from(jsonDecode(utf8.decode(base64Decode(s))).map((k, v) => MapEntry(CustomObject.fromJson(k as Map<String, dynamic>), CustomObject.fromJson(v as Map<String, dynamic>))))',
      );
    });
